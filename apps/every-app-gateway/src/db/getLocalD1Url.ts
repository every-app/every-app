 ...
 * the gateway (previously lived in the now-deleted SDK client surface).
 */
...
export function getLocalD1Url(): string | null {
  ...
  if (!dbFile) {
    const wranglerConfig = parse(
      fs.readFileSync(path.resolve("wrangler.jsonc"), "utf-8"),
    );
    ...
    execSync(
      `npx wrangler d1 execute ${databaseName} --local --persist-to .wrangler/state --command "SELECT 1;"`,
      { stdio: "pipe" },
    );
